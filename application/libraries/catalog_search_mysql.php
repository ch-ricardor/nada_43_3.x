...
class Catalog_search{
	...
	*/
	...
	function search($limit=15, $offset=0)
	{
		...
		if ($sort_by=='proddate')
		{
			// @todo Review this assignation [1]
			// $sort_options[2]=array('sort_by'=>'nation', 'sort_order'=>'asc');
		...
		if ($variable!==FALSE)
		{
			...
			// @todo move the query execution after the if-then-else
	...
	*/
	function _build_study_query()
	{
		...
		//fulltext index name
		// @todo Review Fulltext Index Creation in the schema.sql
		...
		$study_fulltext_index.=',surveys.abbreviation,surveys.kindofdata,surveys.keywords';
