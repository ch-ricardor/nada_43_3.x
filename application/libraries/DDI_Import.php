...
class DDI_Import{
	...
	*/
	function replace($data,$ddi_file_path,$target_survey_id)
	{
		...
		// @todo RRE - Types $id and $target_survey_id are different
	...
	function import_study()
	{
		...
		if(!$id)
		{
			...
			else
			{
				...
				$this->errors[]='DB_ERROR: '.$this->ci->db->error().'<BR />'.$this->ci->db->last_query();
