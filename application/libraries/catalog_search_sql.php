 ...
 * @todo Catalog Search platform independent sqlsrv file was used. (More bytes and newer file in the directory)
 * @todo Object ci renamed to ci_csearch "ci" is used as an object example in the CI references.
 */ 
 ...
 class Catalog_search{
	...
	{
		$this->ci_csearch=& get_instance();
		
		//change default sort if regional search is ON
		if ($this->ci_csearch->config->item("regional_search")=='yes')
		{
			$this->sort_by='nation';
		}
		...
		//use fulltext or not
		if ($this->ci_csearch->config->item("sqlsrv_use_fulltext")===TRUE)
		{
		...
		//$this->ci_csearch->output->enable_profiler(TRUE);
	}
	...
	function _search_count($limit=15, $offset=0)
	{
		...
		// @todo $where_list is destroyed here!
		...
		if ($variable!==FALSE)
		{
			...
			$query=$this->ci_csearch->db->query($query_count);
		}		
		else 
		{
			//study search
			$this->ci_csearch->db->select(" count(surveys.id) as rowsfound ",FALSE);
			$this->ci_csearch->db->from('surveys');
			$this->ci_csearch->db->join('forms f','surveys.formid=f.formid','left');
			if ($repository!='')
			{
				$this->ci_csearch->db->join('survey_repos','surveys.id=survey_repos.sid','left');
			}
			
			if ($where!='') 
			{
				// @todo assumes published=1 (Error in the where construction)
				// $this->ci_csearch->db->where($where);
				$this->ci_csearch->db->where('published',1);
			}
		
			$query=$this->ci_csearch->db->get();
		}
	...
	function search($limit=15, $offset=0)
    {
		...
		{
			//variable search			
			$this->ci_csearch->db->select($study_fields.',varcount, count(*) as var_found',FALSE);
			$this->ci_csearch->db->from('surveys');
			$this->ci_csearch->db->join('forms f','surveys.formid=f.formid','left');
			$this->ci_csearch->db->join('variables v','surveys.id=v.surveyid_fk','inner');
			$this->ci_csearch->db->join('repositories','surveys.repositoryid=repositories.repositoryid','left');
			$this->ci_csearch->db->where('surveys.published',1);
			...
			$this->ci_csearch->db->group_by('surveys.id,surveys.refno,surveys.surveyid,surveys.titl,surveys.nation,surveys.authenty, f.model,link_report,link_indicator, link_questionnaire, surveys.link_da, link_technical, link_study,proddate, surveys.isshared, surveys.repositoryid,varcount, repositories.title, surveys.created,surveys.data_coll_start,surveys.data_coll_end,surveys.changed,surveys.total_views,surveys.total_downloads');
			
			if (trim($this->study_keywords)!=='')
			{
				$this->ci_csearch->db->group_by('k.rank');
			}
			...
			{
				$this->ci_csearch->db->order_by($sort['sort_by'],$sort['sort_order']);
			}
			
			$this->ci_csearch->db->limit($limit,$offset);
			// @todo Query execution at the end of the If-then-Else
			// $query=$this->ci_csearch->db->get();
		}		
		else 
		{
			//study search
			$this->ci_csearch->db->select(" $study_fields ",FALSE);
			$this->ci_csearch->db->from('surveys');
			$this->ci_csearch->db->join('forms f','surveys.formid=f.formid','left');
			$this->ci_csearch->db->join('repositories','surveys.repositoryid=repositories.repositoryid','left');
			$this->ci_csearch->db->where('surveys.published',1);
		...
		// print_r(var_dump($this->ci_csearch->db).'<br>'); die;
		$query=$this->ci_csearch->db->get();
		...
		{			
			$this->ci_csearch->db->join('survey_repos','surveys.id=survey_repos.sid','left');
			$this->ci_csearch->db->where('survey_repos.repositoryid',(string)$this->repo);
		}

		$query_total_surveys=$this->ci_csearch->db->get('surveys')->row_array();
		//$query_total_surveys=$this->ci_csearch->db->query(sprintf('SELECT count(*) as rowsfound from %ssurveys', $this->ci_csearch->db->dbprefix))->row_array();
	...
	*/
	function _build_study_query()
	{
        ...
        $this->ci_csearch->db->join("freetexttable(surveys, (ft_keywords), ".$this->ci_csearch->db->escape($study_keywords).") k","surveys.id=k.[key]","INNER");
	}
	...
	function _build_variable_query()
	{
		...
		if ($this->use_fulltext)
		{
			...
			return sprintf("( freetext((%s),%s) )",$fulltext_index,$this->ci_csearch->db->escape($variable_keywords));
		}
		...
		foreach($keyword_list as $keyword)
		{
			if (strlen($keyword) >=3)
			{
				...
				//wild card search
				$tmp_where[]=sprintf('v.labl like %s',$this->ci_csearch->db->escape('%'.$keyword.'%'));
				$tmp_where[]=sprintf('v.name like %s',$this->ci_csearch->db->escape('%'.$keyword.'%'));
				$tmp_where[]=sprintf('v.qstn like %s',$this->ci_csearch->db->escape('%'.$keyword.'%'));
				$tmp_where[]=sprintf('v.catgry like %s',$this->ci_csearch->db->escape('%'.$keyword.'%'));
			}	
		}	
	...
	*/
	function _build_countries_query()
	{
		...
		foreach($countries  as $country)
		{
			...
			$countries_list[]=$this->ci_csearch->db->escape($country);
		}
	...
	*/
	function get_survey_citation()
	{
		...
		$this->ci_csearch->db->select('sid,count(sid) as total');	
		$this->ci_csearch->db->where("sid in ($surveys)");
		$this->ci_csearch->db->group_by('sid');	
		$query=$this->ci_csearch->db->get('survey_citations');
	...
	function vsearch($limit = 15, $offset = 0)
	{
		...
		//search
		$this->ci_csearch->db->limit($limit, $offset);		
		$this->ci_csearch->db->select("v.uid,v.name,v.labl,v.varID,  surveys.titl as titl,surveys.nation as nation, v.surveyid_FK",FALSE);
		$this->ci_csearch->db->join('surveys', 'v.surveyid_fk = surveys.id','inner');	
		$this->ci_csearch->db->join('forms f','surveys.formid=f.formid','left');
		$this->ci_csearch->db->order_by($sort_by, $sort_order); 
		$this->ci_csearch->db->where($where);
        $this->_build_study_query();


        //get result set
		$result=$this->ci_csearch->db->get("variables as v")->result_array();
		...
		$query_found_rows=$this->ci_csearch->db->query($query_count)->row_array();

		$found_rows=$query_found_rows['rowsfound'];

		$tmp['total']=$this->ci_csearch->db->count_all('variables');
	...
	function v_quick_search($surveyid=NULL,$limit=50,$offset=0)
	{
		...
		//search
		$this->ci_csearch->db->limit($limit, $offset);		
		$this->ci_csearch->db->select("v.uid,v.name,v.labl,v.varID,v.qstn");
		$this->ci_csearch->db->order_by($sort_by, $sort_order); 
		$this->ci_csearch->db->where($where);
		$this->ci_csearch->db->where('surveyid_fk',$surveyid);
		
		//get resultset
		$query=$this->ci_csearch->db->get("variables as v");
	...
	function _build_repository_query()
	{
		...
		if ($repo!='')
		{
			return sprintf('survey_repos.repositoryid = %s',$this->ci_csearch->db->escape($repo));
		}
		return FALSE;
	}
	...
	{	
		...
		{
			//escape country names for db
			$param_list[]=$this->ci_csearch->db->escape($param);
		}
