...
class Menu extends MY_Controller {
	...
	function _search()
	{
		...
		//simple search
		if ($this->input->post_get("keywords") ){
			$filter[0]['field']=$this->input->post_get('field');
			$filter[0]['keywords']=$this->input->post_get('keywords');			
		}		
	...
	function edit($id=NULL)	
	{
		...
		if ($this->config->item("use_html_editor")!=='no')
		{
			//load js rich text editor
	       		$this->template->add_js($this->config->item('javascript_location').'/tiny_mce/tiny_mce.js');
		}
		...
		//validation rules
		// @todo CI 2.x to 3.x Upgrade xss_clean remove
		...
		$this->form_validation->set_rules('url', t('url'), 'trim|required|callback__url_check|max_length[255]');
    	$this->form_validation->set_rules('published', t('published'), 'is_natural|max_length[1]');
    	$this->form_validation->set_rules('target', t('Open_in'), 'is_natural|max_length[2]');
		$this->form_validation->set_rules('weight', t('weight'), 'is_natural|max_length[3]');
	...
	*/
	function add_link($id=NULL)
	{
		...
		//validate form input
		// @todo CI 2.x to 3.x Upgrade remove xss_clean
		$this->form_validation->set_rules('title', t('title'), 'trim|required|max_length[255]');
		$this->form_validation->set_rules('url', t('url'), 'trim|required|callback__url_check|max_length[255]');
    	$this->form_validation->set_rules('published', t('published'), 'numeric');
    	$this->form_validation->set_rules('target', t('Open_in'), 'numeric');
		$this->form_validation->set_rules('weight', t('weight'), 'numeric|max_length[4]');
	...
	*/
	...
	{			
		...
		{
			//redirect page url
			$destination=$this->input->post_get('destination');
	...
	*/
	...
	{		
		...
		$this->template->add_css($this->config->item('javascript_location').'/jquery/themes/base/jquery.ui.theme.css');
		$this->template->add_js($this->config->item('javascript_location').'/jquery/ui/minified/jquery.ui.core.min.js');
		$this->template->add_js($this->config->item('javascript_location').'/jquery/ui/minified/jquery.ui.widget.min.js');
		$this->template->add_js($this->config->item('javascript_location').'/jquery/ui/minified/jquery.ui.mouse.min.js');
		$this->template->add_js($this->config->item('javascript_location').'/jquery/ui/jquery.ui.sortable.js');
