...
class Catalog extends MY_Controller {
	...
	function index()
	{
		...
		$this->template->add_js($this->config->item('javascript_location').'/catalog_admin.js');
	...
	*/
	...
	function upload()
	{
		redirect("admin/catalog/add_study");
		return;
	...
	function add_study()
	{
		...
		// @todo RRE Review Import Process
		//import DDI file
	...
	**/
	function replace_ddi($surveyid=NULL)
	{
		...
		if (!is_numeric($surveyid))
		{
			show_error("Study ID_INVALID");
		}

		$this->acl->user_has_study_access($surveyid);

		// at least one rule require for validation class to work
		// @todo Review Form Validation
		// $this->form_validation->set_rules('id', 'id', 'trim|required|isnumeric');
		...
		if ($this->form_validation->run() == TRUE)
		{
			...
			if ($_FILES['userfile']['size']==0)
			{
				...
				// @todo removed redirect ends the execution.
				// exit;
			}
			// catalog folder path
			// $catalog_root=$this->config->item("catalog_root");
			// @todo catalog folder path TMP
			$catalog_root=$this->config->item("catalog_root").'/tmp';

			if (!file_exists($catalog_root) )
			{
				show_error("CATALOG_ROOT_NOT_FOUND Validate Configuration");
			}
