...
class Citations extends MY_Controller {
	...
	*/
	...
	function edit($id=NULL)
	{
		...
		$this->template->add_css($this->config->item('javascript_location').'/jquery/themes/base/minified/jquery-ui.min.css');
		...
		*/
		...
		$this->form_validation->set_rules('authors', 'Authors', 'trim|max_length[255]');
		$this->form_validation->set_rules('url', 'URL', 'trim|max_length[255]');
		$this->form_validation->set_rules('volume', 'Volume', 'trim|max_length[45]');
		$this->form_validation->set_rules('issue', 'Issue', 'trim|max_length[45]');
		$this->form_validation->set_rules('pub_year', 'Year', 'trim|max_length[4]|is_numeric');
		$this->form_validation->set_rules('doi', 'DOI', 'trim|max_length[45]');
		$this->form_validation->set_rules('flag', t('flag_as'), 'trim|max_length[45]');
		$this->form_validation->set_rules('published', t('published'), 'trim|is_numeric');
		...
		if ($this->input->post("select")==FALSE)
		{
			...
			else
			{
				...
				if ($id!=NULL && $this->input->post("submit")===NULL)
				{
