...
// RRE -- Cache Test, forcing the cache status
// $cache_disabled=0;
		switch($section)
		{
			case 'info':
...
// RRE -- Testing the content of var html
// echo '<div> Var type '.gettype($html).'</div>';
					if ($html===FALSE)
					{
						$html=$this->DDI_Browser->get_overview_html($ddi_file,$language);
						$html=html_entity_decode(url_filter($html));
		...
		*/
		...
		{
			$this->template->add_css($this->config->item('javascript_location').'/tree/jquery.treeview.css');
			$this->template->add_css('themes/ddibrowser/ddi.css');
			$this->template->add_js($this->config->item('javascript_location').'/tree/jquery.treeview.pack.js');
			$this->template->add_js($this->config->item('javascript_location').'/ddibrowser.js');

			// array('sidebar'=>$sidebar, 'body'=>$html,'survey_title'=>$survey_title);
			$data['sidebar']='';
			if($show_data_menu==TRUE || $show_study_menu==TRUE)
			{
				$data['sidebar']=$this->get_sidebar_html($id,$show_study_menu,$show_data_menu);
				// $this->load->view('ddibrowser/sidebar_flat',$options,TRUE);
			}
...
// RRE -- Bug xslt_helper Testing sidebar Error
// $data=FALSE;
// echo 'ddir-file: '.$ddi_file;die;
		if ($data===FALSE)
		{
			$data['sidebar_items']='';
			...
			{
// RRE --
// @todo Review -> XML processing from Memory xslt_helper.php Error
	...
	**/
	...
	private function review_study($id)
	{
		...
		$this->template->add_js($this->config->item('javascript_location').'/jquery/ui/minified/jquery-ui.custom.min.js');
