...
class Catalog extends MY_Controller {
	...
	public function __construct()
	{
		...
		*/
		$this->topic_search=($this->config->item("topic_search")===NULL) ? 'no' : $this->config->item("topic_search");
		$this->regional_search=($this->config->item("regional_search")===NULL) ? 'no' : $this->config->item("regional_search");
		$this->center_search=($this->config->item("center_search")===NULL) ? 'no' : $this->config->item("center_search");
		$this->collection_search=($this->config->item("collection_search")===NULL) ? 'no' : $this->config->item("collection_search");
		$this->da_search=($this->config->item("da_search")===NULL) ? 'no' : $this->config->item("da_search");
	...
	**/
	...
	function index()
	{
		...
		/**
		   @todo Configure the loation for the javascript files defining
		   $this->config->item('javascript_location');
		**/
/*
...
*/
		...
		// @todo RRE Enable to have a optional path to javascripts
		$this->template->add_js($this->config->item('javascript_location').'/datacatalog.js');
		$this->template->add_css($this->config->item('javascript_location').'/jquery/themes/base/jquery-ui.css');
		$this->template->add_js($this->config->item('javascript_location').'/jquery/ui/jquery.ui.core.js');
		$this->template->add_js($this->config->item('javascript_location').'/jquery/ui/jquery.ui.position.js');
		$this->template->add_js($this->config->item('javascript_location').'/jquery/ui/jquery.ui.widget.js');
		$this->template->add_js($this->config->item('javascript_location').'/jquery/ui/jquery.ui.button.js');
		$this->template->add_js($this->config->item('javascript_location').'/jquery/ui/jquery.ui.tabs.js');
		$this->template->add_js($this->config->item('javascript_location').'/jquery/ui/jquery.ui.dialog.js');
		$this->template->add_js($this->config->item('javascript_location').'/jquery.scrollTo-min.js');
		$this->template->add_js($this->config->item('javascript_location').'/jquery.blockui.js');
	...
	function _search()
	{
		...
		if($this->topic_search=='yes')
		{
			...
			if ($vid!==FALSE && is_numeric($vid))
			{
				...
				//get topics by vid
				$data['topics']=$this->Vocabulary_model->get_terms_array($vid,$active_only=TRUE);
				//$this->Vocabulary_model->get_tree($vid);
				$data['topic_search']=TRUE;
			}
			else
			{
	...
	*/
	function vsearch($surveyid=NULL)
	{
		...
		$params=array(
			'study_keywords'=>$this->input->post_get('sk'),
			'variable_keywords'=>$this->input->post_get('vk'),
			'variable_fields'=>$this->input->post_get('vf'),
			'countries'=>$this->input->post_get('country'),
			'topics'=>$this->input->post_get('topic'),
			'from'=>$this->input->post_get('from'),
			'to'=>$this->input->post_get('to'),
			'sort_by'=>$this->input->post_get('sort_by'),
			'sort_order'=>$this->input->post_get('sort_order'),
			'repo'=>$this->input->post_get('repo')
		);
		$this->load->library('catalog_search',$params);
	...
	**/
	function compare($option=NULL, $format=NULL)
	{
		...
		$this->template->add_js($this->config->item('javascript_location').'/dragtable.js');
