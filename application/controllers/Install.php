...
//	function Install()
	...
	function index()
	{
		...
		if ($data['db_connect']!==FALSE)
		{
			...
			// 4. test if database is already installed
			$this->is_already_installed();
		}


		// 5. test the required extensions
	...
	function create_user()
	{
		...
		// @todo CI 3.x Upgrade Remove the xss_clean form validation Upgrade 2.x to 3.x guide
		//validate form input
/*
...
*/
		$this->form_validation->set_rules('username', t('username'), 'max_length[20]|callback_username_exists');
		$this->form_validation->set_rules('email', t('email'), 'max_length[100]|required|valid_email|callback_email_exists');
		$this->form_validation->set_rules('first_name', t('first_name'), 'max_length[20]|required');
		$this->form_validation->set_rules('last_name', t('last_name'), 'max_length[20]|required');
		$this->form_validation->set_rules('phone1', t('phone'), 'max_length[20]|xss_clean|trim');
		$this->form_validation->set_rules('company', t('company'), 'max_length[255]');
		$this->form_validation->set_rules('password', t('password'), 'required|min_length['.$this->config->item('min_password_length').']|max_length['.$this->config->item('max_password_length').']|matches[password_confirm]');
		$this->form_validation->set_rules('password_confirm', t('password_confirmation'), 'required');
	...
	function is_already_installed() {
		...
		if ($this->db->dbdriver=='' || $this->db->username=='' || $this->db->database=='')
		{
        	...
        	*/
		// Check if the db has an active connection
		...
		if (!isset($this->db->conn_id)){
			...
			switch($this->db->dbdriver)
			{
				...
				case 'sqlsrv':
					...
					break;
				default:
					show_error('INSTALLER::database not supported ->'. $this->db->dbdriver);
			}
		...
		// @todo RRE Table validation, return false, then validate the call for TRUE - FALSE
	        if (! $this->db->table_exists('configurations')){
        	        log_message('info', 'Database empty requires installation');
	                return FALSE;
        	}
		...
		if (isset($query))
		{
			...
			if (isset($result['name']))
			{
				...
				show_error(t('System already installed').' - <a href="'.site_url().'">'.t('return_to_site').'</a>');
				exit;
			}
		}
	...
	*/
	function _test_connection()
	{
		...
		// @todo RRE Test if there is an active Database connection
		if (isset($this->db->conn_id)){
        	    return TRUE;
		}
	...
	*/
	function _create_tables()
	{
		...
		// @todo PHP 7.x schema file for the new drivers
...
// RRE ----------------------------------------------------------------------
/*
	@todo CI 3.x Upgrade Test Functions not defined
